...
def LGBM_train(t1,t2,i,on_off):
    ...
    list_1=[name for name in col if name.startswith('median_')]
    train.drop(['link_ID','in_first', 'in_second', 'in_third','in_forth','out_first', 'out_second','out_third','out_forth','time_interval_begin','time_interval_week'],inplace=True,axis=1)
    #train.drop(list_1,inplace=True,axis=1)
    ...
    if on_off == "off":
        ...
        test.to_csv('/home/wangxuefei/Downloads/sub_data/model_%d_offline.txt' % (i), index=False)
    ...
    if on_off == "on":
        ...
        result = runLGBM(train, train_label, test, test_lable,on_off)
        print("写入文件...")
        test_result = pd.read_csv('/home/wangxuefei/Downloads/data/%s.txt' % (t2), low_memory=False)
        ...
        gy_teample_sub.to_csv('/home/wangxuefei/Downloads/sub_data/444.txt', index=False)

        gy_teample_sub[['link_ID', 'date_time', 'time_interval', 'travel_time']].to_csv('/home/wangxuefei/Downloads/sub_data/Fighting666666_0915_4.txt', sep='#', index=False, header=False)
